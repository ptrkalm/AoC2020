...
pub fn part1(input: &str) -> i32 {
    let pairs = retrieve_key_values(input);
    let mut bags_map: HashMap<String, Vec<String>> = HashMap::new();

    for pair in pairs {
        let key = pair.0.to_owned();
        let values = pair.1.iter().map(|&value| {
            let words = value.split_whitespace().collect::<Vec<&str>>();
            format!("{} {}", words[1], words[2])

        }).collect::<Vec<String>>();
        for value in values {
            bags_map.push(value, key.clone());
        }      
    }

    let mut valid_bags: HashSet<String> = HashSet::new();
    count1(&mut valid_bags, &String::from("shiny gold"), &bags_map);

    (valid_bags.len() - 1) as i32
}

fn count1(acc: &mut HashSet<String>, bag: &String, map: &HashMap<String, Vec<String>>) {
    acc.insert(bag.clone());
    match map.get(bag) {
        Some(bags) => for bag in bags {
            count1(acc, bag, map);
        },
        None       => {}
    }
...
pub fn part2(input: &str) -> i32 {
    let pairs = retrieve_key_values(input);
    let mut bags_map: HashMap<String, Vec<(i32, String)>> = HashMap::new();
    
    for pair in pairs {
        let key = pair.0.to_owned();
        let values = pair.1.iter().map(|&value| {
            let words = value.split_whitespace().collect::<Vec<&str>>();
            (words[0].parse::<i32>().unwrap_or(0), format!("{} {}", words[1], words[2]))
        }).collect::<Vec<(i32, String)>>();
        for value in values {
            bags_map.push(key.clone(), value);
        }
    }
    
    count2(&String::from("shiny gold"), &bags_map) - 1
}

fn count2(bag: &String, map: &HashMap<String, Vec<(i32, String)>>) -> i32 {
    let mut count = 1;
    match map.get(bag) {
        Some(bags) => for bag in bags {
            count += bag.0 * count2(&bag.1, map);
        },
        None       => {}
    }
    count
...
fn retrieve_key_values(input: &str) -> Vec<(&str, Vec<&str>)> {
    let lines: Vec<Vec<&str>> = 
        input
        .lines()
        .map(|line| line.split(" bags contain ").collect::<Vec<&str>>())
        .collect();

    let mut pairs: Vec<(&str, Vec<&str>)> = vec!();
    for line in lines {
        let key = line[0];
        let values = line[1].split(",").collect::<Vec<&str>>();
