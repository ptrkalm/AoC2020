...
pub fn part1(input: &str) -> i32 {
    ...
    for passport in input.split("\n\n") {
        ...
        for field in passport.split_whitespace() {
            fields.insert(field.split(":").next().unwrap());
        }
        match (fields.len(), fields.contains("cid")) {
            (8, _)      => count += 1,
            (7, false)  => count += 1,
            _           => {}
        }
...
pub fn part2(input: &str) -> i32 {
    ...
    for passport in input.split("\n\n") {
        ...
        match (
            fields.get("byr").unwrap_or(&"").is_valid_year(1920, 2002),
...
impl Valid for &str {
    fn is_valid_year(&self, min: i32, max: i32) -> bool {
        let value = self.parse::<i32>().unwrap_or(0);
        value >= min && value <= max
    }
    ...
    fn is_valid_height(&self) -> bool {
        ...
        let value = String::from(&self[0..self.len() - 2]).parse::<i32>().unwrap_or(0);
        let system = String::from(&self[self.len() - 2..self.len()]);

        match (system.as_str(), value >= 150 && value <= 193, value >= 59 && value <= 76)  {
            ("cm", true, _) => true,
            ("in", _, true) => true,
            _    => false
        }
