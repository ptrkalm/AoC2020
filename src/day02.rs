...
fn is_valid1(input: &str) -> bool {
    let info   = input.split_whitespace().collect::<Vec<&str>>();
    let range  = info[0].split("-").map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>();
    let letter = info[1].chars().next().unwrap();
    let count  = info[2].chars().filter(|&x| x == letter).count() as usize;
...
fn is_valid2(input: &str) -> bool {
    ...
    let range  = info[0].split("-").map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>();
    let letter = info[1].chars().next().unwrap();
    let text = info[2].chars();

    (text.clone().nth(range[0] - 1).unwrap() == letter) ^ (text.clone().nth(range[1] - 1).unwrap() == letter)
}
