...
fn solve(grid: Vec<Vec<char>>, func: &dyn Fn(i32, i32, &Vec<Vec<char>>) -> u8, n: u8) -> i32 {
    ...
    let mut next_grid = prev_grid.clone();
        for i in 0..width {
            for j in 0..height {
                match prev_grid.peek(i as i32, j as i32) {
                    'L' => if func(i as i32, j as i32, &prev_grid) == 0 { next_grid.updt(i, j, '#'); changed = true; },
                    '#' => if func(i as i32, j as i32, &prev_grid) >= n { next_grid.updt(i, j, 'L'); changed = true; },
                    _   => { }
                }
            }
        }
...
fn adj(i: i32, j: i32, grid: &Vec<Vec<char>>) -> u8 {
    (grid.peek(i - 1, j) == '#') as u8 +
    (grid.peek(i + 1, j) == '#') as u8 +
    (grid.peek(i, j - 1) == '#') as u8 +
    (grid.peek(i, j + 1) == '#') as u8 +
    (grid.peek(i - 1, j - 1) == '#') as u8 +
    (grid.peek(i - 1, j + 1) == '#') as u8 +
    (grid.peek(i + 1, j - 1) == '#') as u8 +
    (grid.peek(i + 1, j + 1) == '#') as u8
}

fn see(i: i32, j: i32, grid: &Vec<Vec<char>>) -> u8 {
    let mut x = 1;
    while grid.peek(i - x, j) == '.' { x += 1 }
    let lt = (grid.peek(i - x, j) == '#') as u8; x = 1;
    while grid.peek(i + x, j) == '.' { x += 1 }
    let rt = (grid.peek(i + x, j) == '#') as u8; x = 1;

    while grid.peek(i, j - x) == '.' { x += 1 }
    let up = (grid.peek(i, j - x) == '#') as u8; x = 1;
    while grid.peek(i, j + x) == '.' { x += 1 }
    let dn = (grid.peek(i, j + x) == '#') as u8; x = 1;
    
    while grid.peek(i - x, j - x) == '.' { x += 1 }
    let up_lt = (grid.peek(i - x, j - x) == '#') as u8; x = 1;
    while grid.peek(i - x, j + x) == '.' { x += 1 }
    let up_rt = (grid.peek(i - x, j + x) == '#') as u8; x = 1;

    while grid.peek(i + x, j - x) == '.' { x += 1 }
    let dn_lt = (grid.peek(i + x, j - x) == '#') as u8; x = 1;
    while grid.peek(i + x, j + x) == '.' { x += 1 }
    let dn_rt = (grid.peek(i + x, j + x) == '#') as u8;

    lt + rt + up + dn + up_lt + up_rt + dn_lt + dn_rt
}

trait Day11 {
    fn peek(&self, i: i32, j: i32) -> char;
    fn updt(&mut self, i: usize, j: usize, c: char);
    fn count(&self, c: char) -> i32;
}

impl Day11 for Vec<Vec<char>> {
    fn peek(&self, i: i32, j: i32) -> char {
        let width  = self.get(0).unwrap().len() as i32;
        let height = self.len() as i32;

        match (i < 0, i >= width, j < 0, j >= height) {
            (false, false, false, false) => *self.get(j as usize).unwrap().get(i as usize).unwrap(),
            _                            => 'L'
        }
    }

    fn updt(&mut self, i: usize, j: usize, c: char) {
        self.get_mut(j).unwrap()[i] = c;
    }
